//This is my large version of app.js that we coded. I am refactoring to make it more readable using the express router.
...
//let's do a delete now. it is pretty similar to a put req.
//CRUCIAL to understand---> app.put, app.post, and app.delete all may have the same route, but they do very different things!
//they are the same path, but they do diff things
//the point of the below method is to remove the person from the array, so very similarly to the put, we need to get all of their info, but then remove it.
//in the put, we destructure to get the person's id, here we aren't destructuring, so we pass in the req.params.id instead!
app.delete('/api/people/id:id', (req, res)=>{
...
///When we set up an API, we can test it with Postman! Remember this so we can test our code.

//Our app.js is getting really big! What do we do?
//Use Express router